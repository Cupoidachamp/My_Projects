...
-- Write your SQL solution here
SELECT ca.CategoryName, ROUND(Avg(p.Price),2) AS AvgPrice
FROM products AS p JOIN categories as ca
...
HAVING TotalValue = (
    SELECT 
        MAX(TotalPerOrder)
    FROM (
			SELECT o.OrderID, SUM(od.Quantity*p.Price) AS TotalPerOrder
			FROM orders AS o
			JOIN orderdetails AS od ON o.OrderID = od.OrderID
			JOIN products AS p ON od.ProductID = p.ProductID
			GROUP BY o.OrderID
